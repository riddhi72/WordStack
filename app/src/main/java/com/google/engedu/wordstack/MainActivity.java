...
public class MainActivity extends AppCompatActivity {
    ...
    public boolean onStartGame(View view) {
        ...
        while (j<word1.length() && k<word2.length())
        {
            ...
            if(index==0)
                scrambled_word += word1.charAt(j++);
            else
                scrambled_word += word2.charAt(k++);
        }
        if(j==word1.length())
        ...
        for (int i=scrambled_word.length()-1; i>=0; i--)
            stackedLayout.push(new LetterTile(this, scrambled_word.charAt(i)));
        return true;
    }
